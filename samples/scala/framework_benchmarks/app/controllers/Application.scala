...
// println("JSON")
  ...
  def db(queries: Int) = PredicatedAction(isDbAvailable, Future(ServiceUnavailable)) {
    Action.async {
  ...
  def fortunes() = PredicatedAction(isDbAvailable, Future(ServiceUnavailable)) {
    Action.async {
  ...
  def update(queries: Int) = PredicatedAction(isDbAvailable, Future(ServiceUnavailable)) {
    Action.async {
        val random = ThreadLocalRandom.current()
